...
export class ChangePasswordComponent extends BaseComponent implements OnInit {
  ...
  getEntity() {
    if (this.id) {
      this.service.Get(this.id).subscribe(r => {
        ...
        this.dialog.showSwalSuccesAlert(`Thay Đổi Mật Khẩu`, MessageConstant.FAILURE_REQUEST);
      });
    }
  }
  ...
  save() {
    ...
    this.service.ChangePassword(this.model).subscribe(result => {
      if (result) {
        this.Submitting = false;
        this.dialog.showSwalSuccesAlert(`Thay Đổi Mật Khẩu`, MessageConstant.EDIT_SCCCESS_CONST);
      ...
      this.dialog.showSwalErrorAlert(`Thay Đổi Mật Khẩu`, strMessage);
    });
